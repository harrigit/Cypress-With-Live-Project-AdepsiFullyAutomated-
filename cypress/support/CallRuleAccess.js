Cypress.Commands.add("CreateCallRule", () => {
  ...
  function RandomName() {
    const Name =
      "ABMSDNDsdsfbhdasdsdhkfsquiwafzdxvcbzxhjdfgszdjgwiiyewrtyibvzmhgDYfsdfg668752223999885500004456555544huzsgxrUWFdteayfsrtQEGasafQUIPRXDFSHGEFSRHDGF";
    const Namelength = 10;
    let result = "";
  ...
  cy.get('[placeholder="Description..."]')
    .click()
    .type("Description Is Written By Through Automation Code ");
  ...
  cy.get(
    "#flush-collapseOne > .accordion-body > .row.mt-3 > .mt-3 > .col-auto > .btn"
  ).click();
...
Cypress.Commands.add("updateCallRule", () => {
  cy.get(".material-icons.arrow-rit.ng-star-inserted").click();
  cy.get('[routerlink="/home/settings/calls/rules/all"]').click();
  cy.wait(5000);
  cy.get(
    ":nth-child(2) > .text-end > .btn-group > .btn-outline-secondary"
  ).click();
  cy.wait(3000);
  cy.get(":nth-child(1) > :nth-child(1) > .input-group > .form-control")
    .click()
    .type("updated");
  //cy.get(":nth-child(3) > .input-group > .btn").click();
  cy.get(".btn-primary").click();
  cy.wait(3000);
});
Cypress.Commands.add("deleteCallRule", () => {
  cy.get(".material-icons.arrow-rit.ng-star-inserted").click();
  cy.get('[routerlink="/home/settings/calls/rules/all"]').click();
  cy.wait(5000);
  cy.get(
    ":nth-child(1) > .text-end > .btn-group > .btn-outline-danger > .mat-icon"
  ).click();
